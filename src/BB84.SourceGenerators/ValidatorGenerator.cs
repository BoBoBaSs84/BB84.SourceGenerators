...
public sealed class ValidatorGenerator : IIncrementalGenerator
{
	...
	private void Execute(SourceProductionContext context, (ClassDeclarationSyntax ClassSyntax, SemanticModel SemanticModel)? input)
	{
		...
		AppendPartialClass(sb, ctx.ClassName, ctx.Accessibility, validatedProperties);
	...
	private static void AppendPartialClass(SourceBuilder sb, string className, string accessibility, ImmutableArray<PropertyValidationInfo> validatedProperties)
	{
	...
	{
		sb.OpenBrace();
		sb.AppendLine($"if (!errors.ContainsKey(\"{propertyName}\"))");
		sb.Indent();
		sb.AppendLine($"errors[\"{propertyName}\"] = new List<string>();");
		sb.Outdent();
		sb.AppendLine($"errors[\"{propertyName}\"].Add(\"{GeneratorHelpers.EscapeString(message)}\");");
		sb.CloseBrace();
	}
