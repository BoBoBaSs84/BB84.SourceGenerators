...
public sealed class ValidatorGenerator : IIncrementalGenerator
{
	...
	private void Execute(SourceProductionContext context, (ClassDeclarationSyntax ClassSyntax, SemanticModel SemanticModel)? input)
	{
		...
		AppendPartialClass(sb, className, accessibility, validatedProperties);

		sb.AppendLine("}");

		context.AddSource($"{className}.Validator.g.cs", sb.ToString());
	}

	private static void AppendPartialClass(StringBuilder sb, string className, string accessibility, ImmutableArray<PropertyValidationInfo> validatedProperties)
	{
		sb.AppendLine($"  {accessibility} partial class {className}");
		sb.AppendLine("  {");
		sb.AppendLine("    /// <summary>");
		sb.AppendLine("    /// Validates the current instance and returns a list of validation errors.");
		sb.AppendLine("    /// </summary>");
		sb.AppendLine("    /// <returns>A list of validation error messages. An empty list indicates a valid instance.</returns>");
		sb.AppendLine("    public List<string> Validate()");
		sb.AppendLine("    {");
		sb.AppendLine("      List<string> errors = new List<string>();");
	...
	private static void AppendValidationRule(StringBuilder sb, string propertyName, string typeName, bool isCollection, bool isArray, ValidationRule rule)
	{
		sb.AppendLine();
	...
	private static void AppendRequiredValidation(StringBuilder sb, string propertyName, string typeName, ValidationRule rule)
	{
		...
		sb.AppendLine($"        errors.Add(\"{EscapeString(message)}\");");
	}

	private static void AppendRangeValidation(StringBuilder sb, string propertyName, bool isCollection, bool isArray, ValidationRule rule)
	{
		if (isCollection)
		{
	...
	private static void AppendStringLengthValidation(StringBuilder sb, string propertyName, ValidationRule rule)
	{
		if (rule.MinValue is not null)
		{
			...
			sb.AppendLine($"        errors.Add(\"{EscapeString(minMessage)}\");");
		}
		else
		{
	...
	private static void AppendMinLengthValidation(StringBuilder sb, string propertyName, ValidationRule rule)
	{
	...
	private static void AppendMaxLengthValidation(StringBuilder sb, string propertyName, ValidationRule rule)
	{
	...
	private static void AppendRegularExpressionValidation(StringBuilder sb, string propertyName, ValidationRule rule)
	{
		string message = rule.ErrorMessage ?? $"The field {propertyName} must match the regular expression '{EscapeString(rule.Pattern!)}'.";

		sb.AppendLine($"      if ({propertyName} != null && !Regex.IsMatch({propertyName}, @\"{EscapeVerbatimString(rule.Pattern!)}\"))");
		sb.AppendLine($"        errors.Add(\"{EscapeString(message)}\");");
	}
