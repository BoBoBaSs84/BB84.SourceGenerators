...
public sealed class AutoMapperGenerator : IIncrementalGenerator
{
	...
	private void Execute(SourceProductionContext context, AutoMapperRequest? request)
	{
		...
		foreach ((string outerAccessibility, string outerName, bool outerIsStatic) in request.OuterClasses)
		{
			...
			sb.AppendLine($"{indent}{outerAccessibility} {outerStaticKeyword}partial class {outerName}");
			sb.AppendLine($"{indent}{{");
			baseIndent++;
		}

		string classIndent = new(' ', baseIndent * 2);
		string methodIndent = new(' ', (baseIndent + 1) * 2);
		string bodyIndent = new(' ', (baseIndent + 2) * 2);

		string staticKeyword = request.ContainingTypeIsStatic ? "static " : string.Empty;
		sb.AppendLine($"{classIndent}{request.ContainingTypeAccessibility} {staticKeyword}partial class {request.ContainingTypeName}");
		sb.AppendLine($"{classIndent}{{");

		string methodStaticKeyword = request.MethodIsStatic ? "static " : string.Empty;
		sb.AppendLine($"{methodIndent}{request.MethodAccessibility} {methodStaticKeyword}partial {request.TargetTypeDisplayName} {request.MethodName}({request.SourceTypeFullName} {request.ParameterName})");
		sb.AppendLine($"{methodIndent}{{");

		if (request.SourceIsNullable)
		{
			sb.AppendLine($"{bodyIndent}if ({request.ParameterName} is null)");
			sb.AppendLine($"{bodyIndent}  throw new System.ArgumentNullException(nameof({request.ParameterName}));");
			sb.AppendLine();
		}

		sb.AppendLine($"{bodyIndent}return new {request.TargetTypeFullName}");
		...
		foreach (AutoMapperPropertyMapping mapping in request.PropertyMappings)
		{
			...
			sb.AppendLine($"{bodyIndent}  {mapping.TargetProperty} = {request.ParameterName}.{mapping.SourceProperty}{nullForgiving},");
		}
