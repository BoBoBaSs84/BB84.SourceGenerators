...
public sealed class AbstractionGenerator : IIncrementalGenerator
{
	...
	private static string GenerateAbstractionSource(AbstractionRequest request)
	{
		...
		sb.AppendAutoGeneratedWarning(GeneratorNames.AbstractionGeneratorFullName);
		sb.AppendNullableEnable();
		sb.OpenNamespace(abstractionNamespace);
		sb.AppendLine($"{GeneratorHelpers.GetAccessibilityKeyword(request.AbstractionType.DeclaredAccessibility)} partial interface {request.AbstractionType.Name}");
		sb.OpenBrace();
	...
	private static string GenerateImplementationSource(AbstractionRequest request)
	{
		...
		{
			sb.AppendUsing(abstractionNamespace);
			sb.AppendLine();
		}
		sb.OpenNamespace(implementationNamespace);
		sb.AppendLine($"{GeneratorHelpers.GetAccessibilityKeyword(request.ImplementationType.DeclaredAccessibility)} partial class {request.ImplementationType.Name} : {request.AbstractionType.Name}");
		sb.OpenBrace();
		foreach (IPropertySymbol propertySymbol in propertySymbols)
		{
			sb.AppendLine($"{CreatePropertyComment(request, propertySymbol)}");
