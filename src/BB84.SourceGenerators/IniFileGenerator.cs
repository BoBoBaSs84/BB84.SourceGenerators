...
public sealed class IniFileGenerator : IIncrementalGenerator
{
  ...
  private void Execute(SourceProductionContext context, (ClassDeclarationSyntax ClassSyntax, SemanticModel SemanticModel)? input)
  {
    ...
    AppendReadMethod(sb, className, accessibility, sections);
  ...
  private static void AppendReadMethod(StringBuilder sb, string className, string accessibility, List<SectionInfo> sections)
  {
    ...
    for (int s = 0; s < sections.Count; s++)
    {
      ...
      sb.AppendLine($"        {elsePrefix}if (currentSection == \"{section.SectionName}\")");
      sb.AppendLine("        {");
      ...
      for (int v = 0; v < section.Values.Count; v++)
      {
        ...
        sb.AppendLine($"          {valueElsePrefix}if (key == \"{value.KeyName}\")");
