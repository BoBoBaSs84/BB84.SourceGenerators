...
public sealed class IniFileGenerator : IIncrementalGenerator
{
	...
	private static void AppendReadMethod(StringBuilder sb, string className, string accessibility, List<SectionInfo> sections, string stringComparison, int indentLevel)
	{
		...
		sb.AppendLine($"{innerIndent}/// Reads the specified INI file content and deserializes it into a new instance");
		sb.AppendLine($"{innerIndent}/// of <see cref=\"{className}\"/>.");
		sb.AppendLine($"{innerIndent}/// </summary>");
		sb.AppendLine($"{innerIndent}/// <param name=\"content\">The INI file string content to deserialize.</param>");
		sb.AppendLine($"{innerIndent}/// <returns>A new instance of <see cref=\"{className}\"/> populated from the INI content.</returns>");
		sb.AppendLine($"{innerIndent}public static {className} Read(string content)");
		sb.AppendLine($"{innerIndent}{{");
		sb.AppendLine($"{bodyIndent}{className} result = new {className}();");
		sb.AppendLine($"{bodyIndent}string? currentSection = null;");
		sb.AppendLine($"{bodyIndent}string[] lines = content.Split(new[] {{ \"\\r\\n\", \"\\n\" }}, StringSplitOptions.None);");
		sb.AppendLine();
		sb.AppendLine($"{bodyIndent}for (int i = 0; i < lines.Length; i++)");
		sb.AppendLine($"{bodyIndent}{{");
		sb.AppendLine($"{deepIndent}string line = lines[i].Trim();");
		sb.AppendLine();
		sb.AppendLine($"{deepIndent}if (line.Length == 0 || line[0] == ';' || line[0] == '#')");
		sb.AppendLine($"{deeperIndent}continue;");
		sb.AppendLine();
		sb.AppendLine($"{deepIndent}if (line[0] == '[' && line[line.Length - 1] == ']')");
		sb.AppendLine($"{deepIndent}{{");
		sb.AppendLine($"{deeperIndent}currentSection = line.Substring(1, line.Length - 2).Trim();");
		sb.AppendLine($"{deeperIndent}continue;");
		sb.AppendLine($"{deepIndent}}}");
		sb.AppendLine();
		sb.AppendLine($"{deepIndent}int equalsIndex = line.IndexOf('=');");
		sb.AppendLine($"{deepIndent}if (equalsIndex < 0)");
		sb.AppendLine($"{deeperIndent}continue;");
		sb.AppendLine();
		sb.AppendLine($"{deepIndent}string key = line.Substring(0, equalsIndex).Trim();");
		sb.AppendLine($"{deepIndent}string value = line.Substring(equalsIndex + 1).Trim();");
		sb.AppendLine();

		string sectionIndent = new(' ', (indentLevel + 4) * 2);
		string sectionBodyIndent = new(' ', (indentLevel + 5) * 2);
		string valueBodyIndent = new(' ', (indentLevel + 6) * 2);
		...
		for (int s = 0; s < sections.Count; s++)
		{
			...
			sb.AppendLine($"{sectionIndent}{elsePrefix}if (string.Equals(currentSection, \"{section.SectionName}\", StringComparison.{stringComparison}))");
			sb.AppendLine($"{sectionIndent}{{");

			if (section.NeedsInitialization)
			{
				sb.AppendLine($"{sectionBodyIndent}if (result.{section.PropertyPath} == null)");
				sb.AppendLine($"{valueBodyIndent}result.{section.PropertyPath} = new {section.TypeName}();");
				sb.AppendLine();
			}
			...
			for (int v = 0; v < section.Values.Count; v++)
			{
				...
				sb.AppendLine($"{sectionBodyIndent}{valueElsePrefix}if (string.Equals(key, \"{value.KeyName}\", StringComparison.{stringComparison}))");
				sb.AppendLine($"{valueBodyIndent}result.{section.PropertyPath}.{value.PropertyName} = {GetParseExpression(value, "value")};");
			}
	...
	private static void AppendWriteMethod(StringBuilder sb, string className, List<SectionInfo> sections, bool serializeComments, int indentLevel)
	{
		string indent = new(' ', indentLevel * 2);
		...
		sb.AppendLine($"{indent}/// Writes the specified <see cref=\"{className}\"/> instance to an INI file string.");
		sb.AppendLine($"{indent}/// </summary>");
		sb.AppendLine($"{indent}/// <param name=\"instance\">The <see cref=\"{className}\"/> instance to serialize.</param>");
		sb.AppendLine($"{indent}/// <returns>The serialized INI file string content.</returns>");
		sb.AppendLine($"{indent}public static string Write({className} instance)");
		sb.AppendLine($"{indent}{{");
		sb.AppendLine($"{bodyIndent}StringBuilder sb = new StringBuilder();");
		sb.AppendLine();
		...
		for (int s = 0; s < sections.Count; s++)
		{
			...
			if (s > 0)
				sb.AppendLine($"{bodyIndent}sb.AppendLine();");

			string nullCheck = BuildNullCheck($"instance.{section.PropertyPath}");
			sb.AppendLine($"{bodyIndent}if ({nullCheck})");
			sb.AppendLine($"{bodyIndent}{{");

			if (serializeComments && section.Comment is not null)
				sb.AppendLine($"{deepIndent}sb.AppendLine(\"; {GeneratorHelpers.EscapeString(section.Comment)}\");");

			sb.AppendLine($"{deepIndent}sb.AppendLine(\"[{section.SectionName}]\");");

			foreach (ValueInfo value in section.Values)
			{
				if (serializeComments && value.Comment is not null)
					sb.AppendLine($"{deepIndent}sb.AppendLine(\"; {GeneratorHelpers.EscapeString(value.Comment)}\");");

				sb.AppendLine($"{deepIndent}sb.AppendLine(\"{value.KeyName}=\" + {GetToStringExpression($"instance.{section.PropertyPath}.{value.PropertyName}", value)});");
			}
		...
		sb.AppendLine();
		sb.AppendLine($"{bodyIndent}return sb.ToString();");
		sb.AppendLine($"{indent}}}");
	}

	private static void AppendLoadMethod(StringBuilder sb, string className, List<SectionInfo> sections, int indentLevel)
	{
		string indent = new(' ', indentLevel * 2);
		string bodyIndent = new(' ', (indentLevel + 1) * 2);
		string deepIndent = new(' ', (indentLevel + 2) * 2);
		...
		sb.AppendLine();
		sb.AppendLine($"{indent}/// <summary>");
		sb.AppendLine($"{indent}/// Loads values from the specified <paramref name=\"other\"/> instance into this instance.");
		sb.AppendLine($"{indent}/// </summary>");
		sb.AppendLine($"{indent}/// <param name=\"other\">The source <see cref=\"{className}\"/> instance to copy values from.</param>");
		sb.AppendLine($"{indent}public void Load({className} other)");
		sb.AppendLine($"{indent}{{");
		...
		foreach (SectionInfo section in sections)
		{
			...
			sb.AppendLine($"{bodyIndent}if ({targetNullCheck} && {sourceNullCheck})");
			sb.AppendLine($"{bodyIndent}{{");

			foreach (ValueInfo value in section.Values)
				sb.AppendLine($"{deepIndent}{section.PropertyPath}.{value.PropertyName} = other.{section.PropertyPath}.{value.PropertyName};");
